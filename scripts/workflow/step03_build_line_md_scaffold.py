...
from annulus_topology_utils import edge_layer_type  # noqa: E402
...
def write_topology_constraints_csv(
    output_path: Path,
    records: list[dict[str, float | int]],
    edges: list[tuple[int, int]],
    radius: np.ndarray,
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for edge_id, (i, j) in enumerate(sorted(tuple(sorted(edge)) for edge in edges), start=1):
            layer_delta = abs(int(layer_ids[i]) - int(layer_ids[j]))
            angle_delta = abs(float(np.angle(np.exp(1j * (float(theta[j]) - float(theta[i]))))))
            writer.writerow(
                {
                    "edge_id": edge_id,
                    "cell_id_a": int(records[i]["cell_id"]),
                    "cell_id_b": int(records[j]["cell_id"]),
                    "index_a": int(i),
                    "index_b": int(j),
                    "row_a": int(records[i]["row"]),
                    "col_a": int(records[i]["col"]),
                    "row_b": int(records[j]["row"]),
                    "col_b": int(records[j]["col"]),
                    "initial_layer_a": int(layer_ids[i]),
                    "initial_layer_b": int(layer_ids[j]),
                    "initial_layer_delta": int(layer_delta),
                    "source_radius_a": f"{float(radius[i]):.10f}",
                    "source_radius_b": f"{float(radius[j]):.10f}",
                    "source_theta_a_deg": f"{math.degrees(float(theta[i])):.6f}",
                    "source_theta_b_deg": f"{math.degrees(float(theta[j])):.6f}",
                    "source_angle_delta_deg": f"{math.degrees(angle_delta):.6f}",
                    "source_radius_delta": f"{abs(float(radius[i]) - float(radius[j])):.10f}",
                    "initial_edge_type": edge_layer_type(layer_delta),
                }
            )
...
def save_scaffold_figure(
    ...
    patches = [
        ...
        mpl.patches.Patch(facecolor="#cfcfcf", edgecolor="none", label="long-layer conflict"),
    ]
    ax.legend(handles=patches, loc="upper right", frameon=True, fontsize=8)
    setup_axis(ax, "Step 3 - Center-polar Annulus-adapted Scaffold", float(np.max(layer_edges)) + 0.22)
...
def save_partition_figure(
    ...
    add_direction_labels(ax, float(np.max(layer_edges)) + 0.12)
    setup_axis(ax, "Line.md Step 2 - Center-polar Layer x Angular Midline Partition", float(np.max(layer_edges)) + 0.22)
    fig.savefig(output_path, facecolor="white", bbox_inches="tight")
    plt.close(fig)
...
def run(args: argparse.Namespace) -> None:
    ...
    tangential, radial, ignored = classify_topology_edges(edges, centers, radius, theta, layer_ids)
    save_scaffold_figure(
        args.output_dir / "step03_line_md_scaffold.png", records, centers, radius, theta, layer_edges, tangential, radial, ignored
    )
    write_topology_constraints_csv(args.output_dir / "step03_topology_constraints.csv", records, edges, radius, theta, layer_ids)
    print(f"cells: {len(records)}")
    print(f"layers: {args.layers}")
    print(f"same-layer arc edges: {len(tangential)}")
    print(f"adjacent-layer radial edges: {len(radial)}")
    print(f"long-layer conflict edges: {len(ignored)}")
    print(f"wrote: {args.output_dir / 'step03_line_md_scaffold.png'}")
    print(f"wrote: {args.output_dir / 'step03_topology_constraints.csv'}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Step 3: apply 线.md to center-polar annulus and draw the topology scaffold.")
